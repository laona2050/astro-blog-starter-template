...
## ⚠️ 重要错误记录 - 时区换算错误（2026-03-15）

**踩坑日期：** 2026-03-15

**🔴 问题：** 时区换算混乱，向用户报告错误的时间

**错误场景：**
- 用户消息时间戳：`Sun 2026-03-15 01:21 UTC`
- 实际北京时间：`01:21 UTC + 8 = 09:21 北京时间`
- 我错误报告为：`凌晨 1:21（北京时间）`

**根因：**
- 混淆了 UTC 和北京时间的换算关系
- 没有正确执行 `UTC + 8 = 北京时间`
- 错误地将 UTC 时间直接当作北京时间报告
...
示例：
- UTC 01:22 → 北京 09:22（同一天上午）
- UTC 10:00 → 北京 18:00（同一天傍晚）
- UTC 22:00 → 北京 06:00（第二天早上）
- UTC 17:00 → 北京 01:00（第二天凌晨）
```

**✅ 检查清单（每次报告时间前）：**
- [ ] 确认当前 UTC 时间
- [ ] 执行 `UTC + 8` 换算
- [ ] 如结果≥24，日期 +1 天，时间减 24
- [ ] 向用户报告北京时间（UTC+8）

**预防措施：**
- 系统上下文中已设置 `userTimezone: "Asia/Shanghai"`
- 调用 `date` 命令时直接获取本地时间
- 不确定时，运行 `date '+%Y-%m-%d %H:%M:%S %Z'` 确认
